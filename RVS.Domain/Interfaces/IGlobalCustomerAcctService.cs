...
/// <summary>
/// Service for managing <see cref="CustomerIdentity"/> (global customer account) entities.
...
public interface IGlobalCustomerAcctService
{
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    Task<CustomerIdentity> GetOrCreateAsync(string email, string firstName, string lastName, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <exception cref="KeyNotFoundException">Thrown when the identity is not found.</exception>
    Task<CustomerIdentity> LinkProfileAsync(string identityId, string tenantId, string profileId, CancellationToken cancellationToken = default);
}
