...
| **A-15** | **Operator identity and public policies.** Every Intake page, including the site root, shows the operating entity's legal name (**Arnold Digital Solutions**) and a contact address in a shared footer, with links to a privacy policy (`/privacy`), terms of use (`/terms`) and the texting terms (`/sms-terms`). All three are public, need no login, and link to each other. This is a launch gate, not a feature: toll-free verification (A-14, `#659`) needs a credible business presence that corroborates the filing entity. The policies are legal documents: wording changes need the owner's sign-off. **Retention, as the privacy policy states it:** contact details and service requests are kept while the dealership uses RV Intake, or until the customer asks for deletion; each RV's VIN-keyed service history (the X-2 ledger) is kept indefinitely and may be used in anonymised aggregate form (X-3). Deletion on request is a manual operator task today. Issue `#680`. |
