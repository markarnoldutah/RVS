...
| **A-6** | Attachments: jpeg, png, mp4, m4a, wav. Max 5, 25 MB each. The cap counts every attachment — photos, videos and voice notes together. Each location may set a lower cap (1–5); the API enforces it on both the upload link and the confirm, not only the intake app. Direct browser-to-Blob SAS upload — binaries never transit the API. |
| **A-7** | **Deferred.** Returning customers (matched by email) get name, phone, and known VINs prefilled. The code is built but unreachable: it needs `?token=` on the intake URL, and nothing RVS sends puts one there. Returning RV customers typically come back next season, after the status token has expired, and a browser-stored alternative would leak one customer's details to the next on a shared device. Vehicle history keeps being recorded so the known-vehicle picker can return. See the Sep 21 2026 entry in `RVS_Plan.md`'s decision log (`#673`). |
...
| **A-13** | **Channel-tagged intake links.** Every distribution path routes through `go.rvintake.com/{locationSlug}`, which redirects to the location's intake URL and logs the hit. A `src` query parameter names the channel: `textrepl` (Text Replacement snippet sent from a Recents entry), `quickreply` (iOS Respond with Text / Android Quick Response, sent from an incoming call), `qr` (QR sticker or NFC tag), `mgrapp` (the location link copied from the manager app's Send intake link dialog and pasted wherever the advisor sends it), and no `src` at all for `print` — printed material cannot carry a query string, so its absence *is* the print channel. The redirect never fails: an unknown `src`, an unknown slug, or a hit-log outage all still send the customer to the form. `src` is persisted on the resulting `ServiceRequest` and reported per location. |
| **A-14** | **Advisor-initiated intake invite.** During a live call, a service advisor or mobile tech sends the caller a prefilled, single-use intake link by text or email from the manager app, with the caller's verbal consent. The resulting request is tagged `src=advisor` and attributed to the advisor and the invite. Detail below. |
| **A-15** | **Operator identity and public policies.** Every Intake page, including the site root, shows the operating entity's legal name (**Arnold Digital Solutions**) in a shared footer, with links to a privacy policy (`/privacy`), terms of use (`/terms`) and the texting terms (`/sms-terms`). All three are public, need no login, and link to each other, and each ends with the operator's contact address — the footer itself states identity, the linked policies state contact, one click away. This is a launch gate, not a feature: toll-free verification (A-14, `#659`) needs a credible business presence that corroborates the filing entity. The policies are legal documents: wording changes need the owner's sign-off. **Retention, as the privacy policy states it:** contact details and service requests are kept while the dealership uses RV Intake, or until the customer asks for deletion; each RV's VIN-keyed service history (the X-2 ledger) is kept indefinitely and may be used in anonymised aggregate form (X-3). Deletion on request is a manual operator task today. Issue `#680`, `#695`. |
...
**Intake sets an opt-out, never clears one (`#673`).** With A-7 deferred, the form never shows a customer's stored opt-outs, so an unticked box is not a choice to opt back in. A ticked box sets the opt-out; an unticked box leaves the stored value alone. The confirmation is routed on the **stored** opt-outs after the submission's are applied. The 422 above applies only to the boxes ticked in that submission: a `Text` preference against a stored SMS opt-out is accepted, and the confirmation goes by email. Only an inbound `START` or `UNSTOP` clears an SMS opt-out. **Known gap:** nothing clears an email opt-out. If a customer asks, the fix is a dealer-side toggle in the manager app, which is not built.
...
- **By email instead (issue #693).** When the caller prefers it, the advisor sends the same link by email: first name, email address, and a tick box confirming the caller asked for the link by email (there is no script to read, because STOP/HELP and carrier verification are texting rules). The email comes from the environment's ACS sending domain, says the link works once and when it stops, and carries no STOP/HELP tail. The same rules apply otherwise, with email in place of text: consent recorded on the invite, refusal when email is unavailable in the environment (409) or the tenant's customer record for that address has opted out of email (409), the same shared rate limits, the same token, link and redemption. Email works while texting is disabled, so it is the only sending channel until the toll-free number is verified. There are no email delivery reports: an emailed invite shows as sent or failed, never delivered. The prefill lookup also returns the email address the advisor entered, and the invite records which channel it went by.
...
4. Issue category
5. AI summary, labeled as AI-generated — placed here so the service manager reads the concise recreation of the problem first. Headed **Preliminary assessment**, it also carries, when the model will offer one, a probable cause, a confidence (high / medium / low), **possible fixes** — plural and ordered most plausible first, never a single "recommended" fix, because the unit has not been inspected — and likely parts as generic names (never part numbers or prices). Advisory only, and says so. When the information is too thin the model abstains and only the summary renders. Generated once in the packet pipeline, not on the intake path, and reused on regeneration; a rule-based per-category fallback (low confidence) covers an unavailable model. **From photos:** the same call reads the customer's photos (images only, at most 5, in attachment order) and the block adds what can be seen in them: **data plates** (component, manufacturer, model number, serial number), **fault codes** shown on a display or panel, and **visible observations**. At most 5 of each, and every line cites its photo by position and file name. A model or serial number transcribed off a data plate is a fact from the photo, not a suggested part, so the "never part numbers" rule for likely parts still holds. Only what is visible is reported. The block never covers hidden or internal condition, severity, cost, repair time or warranty, or anything about people, licence plates or addresses. An unreadable field is left out rather than guessed. Photo findings survive an abstention, and the sub-block is omitted when there are none. A photo that arrives after the first generation is not assessed. The rule-based fallback reads no photos
...
| # | Requirement |
|---|---|
| **X-1** | **Customer status page.** `rvintake.com/status/{token}`, anonymous, rate-limited. Shows unit, issue category (the A-5 display name, never the customer's free text — issue #741), submission date, current status (a colored label chip with an icon and a spaced label), the location's phone number, and any manager-authored notes (C-9). The page is free to grow to show whatever is useful for the customer to see about their job. **One hard constraint: it is display-only for the customer.** There is no path for the customer to send anything back — no reply, no inbound message, no file upload. Communication is one-directional, manager → customer. (This does not reopen two-way messaging; see "Explicitly out of scope".) The intake app remembers the token on the device (`localStorage`, with the expiry the API reports) after a submission or a successful status load, and `rvintake.com/status` opens a remembered, unexpired link directly instead of asking for the confirmation number; an unknown or expired token is forgotten. Issue #716. |
...
| **P-5** | **Add location.** Name, optional slug, phone, optional time zone (from the curated list; blank renders the packet's Received line in UTC), 1–10 packet recipients. The stored name always leads with the tenant's business name: `Tucson` for tenant `Blue Compass` is saved as `Blue Compass - Tucson`. A name that already starts with the business name is not prefixed twice. The generated slug is unchanged (`blue-compass-tucson`). |
...
| **P-7** | **Security.** Every `api/admin/*` endpoint requires the `platform:tenants:manage` permission **and** a caller `sub` on `Admin:AllowedUserIds`; otherwise 403. MFA is required on the admin Auth0 account. A dedicated Auth0 M2M application holds only `read:users create:users update:users delete:users update:users_app_metadata read:roles read:role_members create:role_members delete:role_members create:user_tickets`, and its secret lives only in Key Vault. Admin writes are logged (who, what, tenantId); passwords and ticket URLs never are. Missing Auth0 settings fail loudly — there is no no-op fallback. |
...
| **P-9** | **List users.** Every Auth0 user whose `app_metadata.tenantId` is the tenant, with name, email, roles, locations, whether logins are enabled and last sign-in (issue #647). Auth0's user search is eventually consistent: a user added seconds ago may not be listed yet. |
| **P-10** | **Edit user.** Name, role and, for location-scoped roles, locations — replaced together; the P-2 role list applies. The new role is assigned before the user's other `dealer:*` roles are removed, so a failure part-way never leaves the user with no role. Roles outside `dealer:*` and `app_metadata.tenantId` are never changed. The email cannot be edited: delete and re-add. |
| **P-11** | **Disable/enable user.** Blocks or unblocks one user in Auth0, without touching the tenant's access gate (P-4). |
| **P-12** | **Delete user.** Permanently deletes the Auth0 user, after an in-page confirmation. Packet recipients are addresses on the location, not users, and are unaffected. |

P-9 to P-12 act only on users whose `tenantId` matches the route; any other user is a 404. Permissions come from roles only — there is no per-user permission grant. Changes reach a signed-in user when their access token next renews, within an hour.

Out of scope for v1: self-serve signup, bulk import, per-user permission grants, billing automation, branding upload (FS-5), and splitting the shared Auth0 tenant (FS-9).
...
**The STOP/HELP handler does not reopen two-way SMS.** Outbound texting (A-2 confirmations, A-14 invites) receives exactly one kind of inbound traffic: carrier keywords on the sending number. On a toll-free number the carrier already enforces `STOP`: it sends its own reply and blocks further texts from that number to the recipient. RVS mirrors the customer's latest keyword, so its records always match what the carrier enforces. `STOP` and its standard synonyms set the SMS opt-out on that number's customer records, so RVS refuses later sends itself instead of relying on a failed delivery report; `START` and `UNSTOP` clear it again, which is the only way an SMS opt-out is cleared, because intake can set one but never clears one (`#673`). A keyword applies to every dealer's record of that number, because the sending number is shared. `HELP` gets one fixed reply naming RV Intake, pointing at the dealership for help and repeating STOP. It changes nothing, needs no customer record, and is gated by the environment's `Sms:Enabled` like every other send. Consent for that one reply is implied, because the customer texted us first. No other inbound text is read, stored, routed to anyone or answered. There is no inbox and no conversation.
