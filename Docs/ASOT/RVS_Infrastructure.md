...
One record in the `rvintake.com` zone is declared but **not yet live**: `login.rvintake.com`, the Auth0 Universal Login host (`#627`, `#634`). `main.bicep`'s `auth0CnameRecords` writes it from `auth0CustomDomainCnameTarget`, which is empty until the custom domain is created in the Auth0 dashboard and Auth0 mints the verification token — the same out-of-band pattern as the ACS domain above. It is also the one record here with no `-staging` sibling: the Auth0 Free plan includes exactly one custom domain and all three environments share one tenant (`#610`), so both parameter files write the same name and value. Steps are in `Docs/ASOT/Auth0/Auth0-Portal-Configuration-Checklist.md` §6.
