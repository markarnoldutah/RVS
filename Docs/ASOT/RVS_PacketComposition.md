...
**Version:** 1.8 · September 10, 2026
**Scope:** How a service packet is assembled and rendered, end to end. Covers what is built (`#430` composition, `#431` HTML render, `#432` PDF render, `#433` photo SAS resolution, `#434` generation orchestration, `#435` per-location packet config, `#436` DMS paste block, `#437` packet email send, `#438` delivery idempotency + retry/backoff, `#439` hard-bounce recipient disabling — domain + service, `#492` masthead layout pass, `#508` HEIC/HEIF → JPEG transcode on upload, `#516` waiting for in-flight intake uploads before generating, `#521` fitting the email inside the ACS message ceiling, `#562` normalising *every* image upload on confirm — downscale + re-encode, not just HEIC) and the one part of `#439` that is not yet wired (the inbound bounce signal).
...
**Image normalisation — `#508` + `#562` (built).** `AttachmentService.ConfirmAttachmentAsync` normalises **every** image upload *before* it is recorded on the request: it downloads the just-uploaded blob, hands the bytes and declared content type to `IImageTranscoder` (`RVS.Domain/Integrations/`), stores the re-encoded result, best-effort deletes the original when the blob name changed, and records the attachment with the output `ContentType`, matching file extension, and new size. The real implementation is `MagickImageTranscoder` (`RVS.API/Integrations/`) — Magick.NET (ImageMagick, Apache-2.0) with its bundled HEIF delegate (libheif + libde265, LGPL-3.0); the `Magick.NET-Q8-AnyCPU` package ships native builds for the Linux App Service and dev machines alike. Licensing — the LGPL compliance statement and the HEVC-patent note — is in `/THIRD-PARTY-NOTICES.md`, which ships in every deploy artifact.

- **`#508` — the HEIC/HEIF problem.** Those formats render on Apple clients only: the HTML packet shows a blank `<img>`, the PDF's SkiaSharp decoder cannot read them, and emailed `.heic` files will not preview in most desktop mail clients. They are always converted to JPEG, stored under a sibling `.jpg` blob, HEIC original deleted — **even when the JPEG is larger than the HEIC** (a kept `.heic` still would not render).
- **`#562` — every other raster.** `CanNormalize` now also accepts `image/jpeg`, `image/png`, `image/webp`. A full-resolution Android JPEG or point-and-shoot PNG was previously stored untouched at 25 MB-permitting size (`Spec A-6`) and **attached to the packet email at that size**, forcing `#521`'s size fitter to drop photos to stay under the ACS ceiling. (The PDF is not what shrinks: QuestPDF resamples each image to its placed size, so the PDF's size follows image *content*, not source resolution — measured 1.60 MB for six detailed 12 MP photos and 1.64 MB for the same six at 1600 px; ~2.7 MB for ten. `#562`'s payoff is entirely in the emailed attachment set.) Now each upload is downscaled past `ImageTranscodeOptions.MaxEdgePixels` — **default lowered 4096 → 1600** (`#508` chose 4096 to *preserve* native iPhone resolution, so the downscale never fired; 1600 is more than a phone screen or a 210 × 279 mm greyscale page can show and cuts a 12 MP photo's pixel count about sixfold) — EXIF orientation baked in, metadata stripped, re-encoded at quality 82. **A PNG stays PNG** (downscale only): a screenshot or diagram shows JPEG block artifacts on hard edges, so it takes the smaller saving. Every other format re-encodes to JPEG.
- **Measured effect — why 1600** (real 12 MP photo variants, real renderers + fitter at the 9.5 MB default). Before `#562` a photo stored at ~2.4 MB and a six-photo submission attached **2 of 6**. At the default 1600 px a photo stores at ~0.53 MB: six attach **6 of 6** at ~6.4 MB, ten (`Spec A-6` max) attach **8 of 10**. 2048 px was measured first and rejected: six fit with only ~0.5 MB to spare (~9.0 MB), and ten attached 5 of 10. 1280 px attaches 10 of 10 (~8.4 MB) but gives up zoom-in detail a technician may need. The fitter remains the backstop.
- **Never grows a file.** For an already-web-safe raster (JPEG/PNG/WebP) whose re-encode is not smaller than the source, the normaliser returns "no change" and the original is kept — a small, already-optimised image never inflates just by passing through. HEIC/HEIF is exempt, per the first bullet.
- **Failure is safe.** A payload over `MaxSourceBytes` (default 25 MB), an undecodable payload, or a download error keeps the original and logs. A HEIC that could not transcode still shows the packet's labelled placeholder; `#521`'s size fitter stays as the backstop for anything oversized that slips through. Under `Integrations:UseMocks` the seam is `NoOpImageTranscoder` (normalises nothing).
- **The customer's original is not retained.** No repository tracks it, every packet consumer reads the normalised blob, and `Spec A-6` / `X-6` do not require retention — the same deliberate choice `#508` made for HEIC, applied to all formats.
...
**`#521` — the ACS size ceiling, built.** ACS rejects any send whose whole request exceeds **10 MB**, and attachments travel base64 encoded, which inflates them by about a third — Microsoft's own guidance puts the realistic payload of raw attachment bytes near **7.5 MB**. `Spec A-6` allows ten 25 MB uploads, so six full-resolution phone photos attached as originals push the request past the ceiling on their own — six real 12 MP photos measured about 14.9 MB before base64. The PDF is not the cause: QuestPDF resamples embedded images to their placed size, so `#432`'s PDF is roughly 1.5–3 MB for six to ten detailed photos, independent of source resolution — measured 2026-09-10 with the real `PacketPdfRenderer` and real 12 MP phone photos at 1.60 MB for six and 2.83 MB for ten. (An earlier measurement with synthetic noise images put it near 0.5 MB; the noise averaged away in resampling and understated real detail about threefold.) With the fitter at the default budget, a six-real-photo submission sends with the PDF and the first two photos attached. Before this, the send threw, all three `#438` attempts failed identically, and the shop got a service request with no packet.

`PacketEmailSizeFitter` (`RVS.Domain/Packets/`, pure) now decides what can be attached. `PacketGenerationService` calls it with the candidate attachment set, both rendered bodies, and `PacketEmailOptions.MaxRequestBytes` (default `9_500_000` — ACS's 10 MB less a 500 KB margin for JSON escaping and the request envelope), then composes the email from the surviving set. The rules:

- **Cost model.** Each attachment is charged `4 × ceil(bytes / 3)` for base64 plus 256 bytes of JSON overhead; both bodies are charged their UTF-8 length plus a 4 KB envelope allowance. Measuring the real plain-text body means `PacketEmailComposer.BuildPlainTextBody` is now public, so the measurement and the composed message cannot drift apart.
- **The PDF outranks every photo**, and is decided first regardless of where it sits in the input — input order can never cost the packet its printable artifact.
- **Photos survive as a contiguous prefix.** The first photo that does not fit stops the loop rather than skipping ahead to a smaller one, so survivors match the order the packet renders them in (`Spec B-2` item 8, six on page one). Predictable beats maximally packed.
- **A PDF that cannot fit is dropped, and photos still fill the budget.** This should not happen: even a ten-photo PDF is about 3.8 MB on the wire, and the app refuses to start with a budget below 5 MB or above ACS's 10 MB (`PacketEmailBudgetValidator` in `RVS.Domain/Validation/`, run through `PacketEmailOptionsValidator` with `ValidateOnStart`). The upper bound matters more than it looks — a budget over the ceiling would quietly restore the original `#521` failure. So a dropped PDF can only mean a regression: the renderer stopped resampling images, or something large landed in the HTML body. The photos that fit still ship, because they are worth more to the reader than a bare email, and `LogCritical` under `EventId` 521001 flags the broken invariant. The photo-heavy case is the ordinary trim, which logs `LogWarning` and should not alert.
- **Nothing is actually lost.** `#433` already embeds every photo in the HTML body as a 7-day read SAS URL, which is the same blob-plus-SAS pattern Microsoft recommends for oversized mail; the PDF stays in `rvs-attachments`. A dropped attachment costs the reader a local copy, not the picture. A trimmed send still records `status = "Delivered"` — it is a success, not a degraded failure.

Selection is tracked by index rather than by value, because `PacketEmailAttachment` is a record and two same-sized photos with the same name are equal — an attachment must not stand in for its twin. `Spec B-4`.
...
| Normalise *every* image upload, not just HEIC | `#562` | **Built** — `IImageTranscoder` renamed `CanTranscode`/`TranscodeToJpeg` → `CanNormalize`/`Normalize(bytes, contentType)`; `CanNormalize` now accepts `image/jpeg` / `image/png` / `image/webp` alongside HEIC/HEIF. `AttachmentService` downscales + re-encodes every image on confirm (PNG stays PNG, others → JPEG), overwriting in place when the blob name is unchanged. `MaxEdgePixels` default 4096 → **1600** (six real photos attach 6/6 at ~6.4 MB; ten attach 8/10). Never returns a larger file for an already-web-safe raster (HEIC exempt). Removes the reason `#521` had to drop photos; the fitter stays as the backstop |
| Email attachment set fitted to the ACS message ceiling | `#521` | **Built** — `PacketEmailSizeFitter` (pure, Domain) trims the attachment set to `PacketEmailOptions.MaxRequestBytes` (default 9.5 MB of ACS's 10 MB, base64 accounted for): PDF outranks photos, photos kept as a contiguous prefix, a PDF that cannot fit is dropped while photos still fill the budget, logging `LogCritical` 521001. The budget is validated at startup (5 MB to 10 MB, `PacketEmailOptionsValidator` with `ValidateOnStart`). An oversized submission now delivers a trimmed packet instead of failing all three attempts with no packet; dropped photos stay visible inline via their `#433` SAS URLs |
