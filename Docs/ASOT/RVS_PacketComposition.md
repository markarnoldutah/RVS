...
**`#521` — the ACS size ceiling, built.** ACS rejects any send whose whole request exceeds **10 MB**, and attachments travel base64 encoded, which inflates them by about a third — Microsoft's own guidance puts the realistic payload of raw attachment bytes near **7.5 MB**. `Spec A-6` allows ten 25 MB uploads and `#432`'s PDF embeds the photo bytes as well, so a six-photo submission could exceed the ceiling on the PDF alone. Before this, the send threw, all three `#438` attempts failed identically, and the shop got a service request with no packet.
...
- **A PDF that cannot fit takes the whole set with it.** The photo bytes are what made the PDF large, so attaching them instead would not fit either; the email goes out body-only and logs `LogCritical` under `EventId` 521001 — the one case an operator should see, since it usually means the photos need a tighter transcode.
...
| Email attachment set fitted to the ACS message ceiling | `#521` | **Built** — `PacketEmailSizeFitter` (pure, Domain) trims the attachment set to `PacketEmailOptions.MaxRequestBytes` (default 9.5 MB of ACS's 10 MB, base64 accounted for): PDF outranks photos, photos kept as a contiguous prefix, a PDF that cannot fit drops the whole set and logs `LogCritical` 521001. An oversized submission now delivers a trimmed packet instead of failing all three attempts with no packet; dropped photos stay visible inline via their `#433` SAS URLs |
