...
**Version:** 1.6 · September 8, 2026
**Scope:** How a service packet is assembled and rendered, end to end. Covers what is built (`#430` composition, `#431` HTML render, `#432` PDF render, `#433` photo SAS resolution, `#434` generation orchestration, `#435` per-location packet config, `#436` DMS paste block, `#437` packet email send, `#438` delivery idempotency + retry/backoff, `#439` hard-bounce recipient disabling — domain + service) and the one part of `#439` that is not yet wired (the inbound bounce signal).
