...
**Scope:** How a service packet is assembled and rendered, end to end. Covers what is built (`#430` composition, `#431` HTML render, `#432` PDF render, `#433` photo SAS resolution, `#434` generation orchestration, `#435` per-location packet config, `#436` DMS paste block, `#437` packet email send, `#438` delivery idempotency + retry/backoff) and the design of the stage that is not yet (`#439` hard-bounce handling).
...
### 5. Deliver — Feature 3 (`#437`, `#438` built; `#439` planned)
...
| Hard-bounce recipient disabling | `#439` | Planned |
