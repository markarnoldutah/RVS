...
| `GET api/intake/{slug}/invites/{token}` | A-14 invite prefill (#664): first name and phone while the invite is unexpired, unredeemed and for this location. Read-only (opening never redeems). One 404 for every other case, a failed lookup included |
...
`api/locations/{locationId}/intake-invites` — POST, GET (the current advisor's sends for the shift), GET `{id}`, GET `capability` (`{ smsEnabled }`, environment-wide, so the dialog can say texting is off before anyone types; #666) — all behind `CanSendIntakeInvites` (`intake-invites:send`, `Spec A-14`, #663)
...
Self-entry skips consent, texting, the opt-out check and the rate limits, works while texting is off, and returns the prefilled `IntakeUrl` (direct to the intake app) once; the token is not recoverable afterwards.

Rate limits are `InMemoryIntakeInviteRateLimiter`: sliding one-hour windows per advisor, per location and per tenant (`IntakeInvites:MaxPer…PerHour`, 20 / 60 / 100 by default), all-or-nothing under one lock, per process like the tenant SMS limiter. `InMemoryTenantSmsRateLimiter` still caps every SMS behind it. Sends ask ACS for delivery reports; consuming them is #665.
...
| A-7 returning-customer prefill | **Deferred** (Spec A-7, #673) | Code built but unreachable: keyed off `?token=` on the intake URL, which nothing RVS sends carries. The config `?token=` path, Step 1 prefill and the Step 3 known-vehicle picker stay in place, marked deferred |
