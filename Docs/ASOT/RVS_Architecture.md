...
| C-9 customer-facing status notes | **Built** | Issue #500 (overrides the original X-1 "no conversation, no messaging"). `ServiceRequest.CustomerStatusNote` (one nullable embedded note; overwritten on edit). Set/cleared via `PUT api/dealerships/{id}/service-requests/{srId}/status-note` (`CanUpdateServiceRequests`); `CustomerStatusNoteValidator` caps at 280 chars + blocks `< > ; ' " \ \0` → 422. Manager entry in the detail dialog's right column; rendered as an advisory line on `StatusPage.razor`. One-directional; note text never logged |
