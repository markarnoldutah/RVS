...
`api/locations/{locationId}/intake-invites` — POST, GET (the current advisor's sends for the shift), GET `{id}` — all behind `CanSendIntakeInvites` (`intake-invites:send`, `Spec A-14`, #663)
