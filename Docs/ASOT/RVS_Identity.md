...
**Auth0's own session follows the same answer.** Auth0 keeps a separate session cookie on its domain, and the Blazor authentication library silently signs in against it on every auth-state check (a hidden-iframe `signinSilent()`, run continuously, not just at startup), so the app's opt-in alone would not stop Auth0 signing the next person into a shared computer. `KeepSignedInPolicy.ShouldForceLogin` (`RVS.UI.Shared`) decides whether to force Auth0's password prompt; `UnauthorizedAccess.razor` (the automatic redirect when the app-wide fallback policy finds no user) and `LoginDisplay.razor`'s "Log in" button each attach `prompt=login` to their own `InteractiveRequestOptions` when the answer is anything but yes.

**This must be applied per call, never globally, or it breaks the silent-renewal path.** An earlier version of this change added `prompt=login` once to `OidcProviderOptions.AdditionalProviderParameters` (there is a comment marking exactly this spot in `Program.cs`, dating to before issue #498, where a prior attempt at the same thing was reverted). That dictionary becomes the OIDC client's `extraQueryParams`, attached to *every* authorize request the library makes — including the invisible `signinSilent()` iframe check. Auth0 refuses to render its interactive login form inside a frame (anti-clickjacking), so the hidden iframe's navigation gets blocked: the browser stalls for the iframe's timeout (observed ~3-5s) and reports it as a failed request, on every page load and right after every sign-out. `InteractiveRequestOptions.TryAddAdditionalParameter`, used only on the two explicit sign-in call sites above, avoids this: that request always goes straight to a full-page `signinRedirect`, never through the silent/iframe path.

Token renewal is unaffected either way — it uses the refresh token, not a fresh authorize request. Sign-out goes through Auth0's `/oidc/logout` (advertised as `end_session_endpoint`), which ends the Auth0 session — but only if `{origin}/authentication/logout-callback` is an Allowed Logout URL, because Auth0 matches that list exactly and returns 400 otherwise.
