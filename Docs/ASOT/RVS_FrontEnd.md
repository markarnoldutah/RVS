...
Supporting pages: `Confirmation`, `Status` (confirmation-number entry), `StatusPage` (`/status/{Token}` → `GET api/status/{token}`; shows unit, submitted date, status sentence, location `tel:` link, and the manager-authored status note when present — C-9, #500), `Home`, `SmsTerms` (`/sms-terms`), `Privacy` (`/privacy`), `Terms` (`/terms`), `Error`, `NotFound`.
