...
**The step rides in the URL, so the browser's Back button walks the wizard.** The shell carries a `?step=N` query parameter alongside the slug and the other link parameters. The entry link has no step; initialization stamps one in with `replace`, so the first history entry is the wizard's own and Back never returns to an un-stamped URL. Each forward move — Continue, and Review's Edit jump — pushes an entry, and the wizard's own Back button delegates to `history.back()` rather than moving the step itself, so the two buttons are one action and the Forward button keeps working. `WizardStepUrlSync.Reconcile` holds the rules for the reverse direction: when the URL's step and the wizard's disagree, it decides whether the URL wins (Back, Forward, an edited address bar) or gets corrected in place.

Because the step is now something a customer can type, `IntakeWizardState.MaxStepReached` caps what the URL may ask for — it only ever rises, is persisted with the rest of the session, and falls back to `CurrentStep` when reading a session saved before it existed. A `?step=` beyond it is rewritten rather than honoured, so `?step=8` cannot skip the validation of the steps in between, and the rewrite leaves nothing for Forward to reach. Arriving somewhere by pressing Back is not progress, so `GoToStepFromHistoryAsync` never raises the cap, and it drops any pending `ReturnToStepAfterEdit`: that promise belonged to the Continue button of the step Review sent them to edit, and a customer who pressed Back has overridden it. Submission navigates to `/confirmation` with `replace`, so Back cannot return to a wizard that has already been sent.
...
| 2 | `Step2_CustomerInfoStep` | Name, email, phone, then Notification Preferences (text/email opt-outs, texting disclosure linking `/sms-terms`) above Preferred contact method. An opt-out disables its radio and clears a conflicting selection (`Spec A-2`, `#662`). The phone field takes a MudBlazor `RegexMask` built from `PhoneInputMask.Pattern` (`RVS.UI.Shared/Validation`), so letters are refused at the keystroke; `PhoneValidator` still counts the digits (#758). Shows a "pre-filled from your previous visit" banner |
...
| 4 | `Step4_VehicleDetailsStep` | Manufacturer, model, year (editable after decode), extended warranty, approximate purchase date. The "retrieved from VIN lookup" banner reads `VinLookupSucceeded`, which is persisted with the session so a refresh keeps it (#758) |
...
| 7 | `Step7_AttachmentUploadStep` | Up to 10 files at 25 MB, buffered in-browser. Drop zone with an overlaid `InputFile`, whose prompt counts the slots left (`AttachmentSlots.Prompt`). Browser-drawable images get a 144px JPEG thumbnail from `IBrowserFile.RequestImageFileAsync`, made while the file is still readable and held as a small `data:` URL; videos, PDFs and HEIC show an icon (#758) |
...
**`ThemeService` is still not in `RVS.UI.Shared`.** Each app has its own copy at `RVS.Blazor.{Intake,Manager}/Services/ThemeService.cs`. What lives there now is only mode selection and persistence — which of the shared themes is active, and writing that choice to `localStorage` (`rvs-manager-theme-preference`, `rvs-intake-theme-preference`), read back in each app's `Program.cs` before the first render. Intake's persistence arrived with #758; before that, high contrast was lost on reload. No palette is declared in either app.
